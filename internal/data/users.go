...
func ValidateUser(v *validator.Validator, user *User) {
	...
	v.Check(validator.In(user.Role, "organizer", "customer"), "role", "must be either 'organizer' or 'customer'")
}
...
func (m UserModel) GetByEmail(email string) (*User, error) {
	query := `SELECT id, created_at, name, email, password_hash, role, version FROM users WHERE email = $1`
	var user User
	...
	err := m.DB.QueryRowContext(ctx, query, email).Scan(
		&user.ID, &user.CreatedAt, &user.Name, &user.Email, &user.Password.hash, &user.Role, &user.Version,
	)
